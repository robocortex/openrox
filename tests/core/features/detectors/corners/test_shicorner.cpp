//==============================================================================
//
//    OPENROX   : File test_shicorner.cpp
//
//    Contents  : Tests for shicorner.c
//
...
ROX_TEST_SUITE_BEGIN(shicorner)
...
ROX_TEST_CASE_DECLARE(rox::OpenROXTest, test_shicorner_test)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   Rox_Float response = 0.0;
   Rox_Uchar ** image_data = NULL; 

   Rox_Uint radius = 4; // size ROI = 2*radius+1
   ...
   error = rox_shicorner_test ( &response, image_data, v, u, radius );
   ROX_TEST_CHECK_EQUAL ( error, ROX_ERROR_NONE );

   rox_log("shicorner_test response (%d, %d) = %16.16f\n", u, v, response );
