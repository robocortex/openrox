//==============================================================================
//
//    OPENROX   : File rox_example_odometry_single_plane_filter_matse3.c
//
//    Contents  : A simple example program for single plane odometry with
//                filtering of the pose and velocity estimation 
//
...
Rox_Sint main (Rox_Void)
{
   ...
   Rox_Double dt = 30.0/1000.0;
   
   Rox_Filter_MatSE3 filter_matse3 = NULL;
   // predicted pose
   Rox_MatSE3 pose_pre = NULL;
   // estimated velocity
   Rox_Matrix Ah = NULL;
   ...
   error = rox_filter_matse3_set_gain_pose ( filter_matse3, 1/dt );
   if(error) goto function_terminate;

   error =  rox_filter_matse3_set_gain_velocity ( filter_matse3, 0.5/dt );
   if(error) goto function_terminate;
   
   // Create the predicted pose
   error = rox_matse3_new ( &pose_pre );
   if(error) goto function_terminate;
   
   // Create the estimated velocity pose
   error = rox_matrix_new ( &Ah, 4, 4 );
   if(error) goto function_terminate;
   ...
   for (i = 0; i < 100; i++)
   {
      ...
      if(tracked == 1)
      {
         ...
         error = rox_image_rgba_draw_projection_model_single_plane(image_rgba, K, pose_pre, model, blue);
         if(error) goto function_terminate;
         ...
         error = rox_filter_matse3_update ( filter_matse3, pose );
         if(error) goto function_terminate;

         error = rox_filter_matse3_get_predicted_pose ( pose_pre, filter_matse3 );
         if(error) goto function_terminate;

         error = rox_filter_matse3_get_estimated_velocity ( Ah, filter_matse3 );
         if(error) goto function_terminate;

         printf("Estimated velocity :\n");
         rox_matrix_print(Ah);
      ...
      else
      {
         ...
         if(error == 0)
         {
            ...
            // Init filter
            error = rox_filter_matse3_init_pose ( filter_matse3, pose );
            if(error) goto function_terminate;
...
function_terminate:
   ...
   rox_matrix_del(&K);
   rox_matse3_del(&pose);
   rox_matse3_del(&pose_pre);
   rox_matrix_del(&Ah);
   rox_filter_matse3_del(&filter_matse3);
