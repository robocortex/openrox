...
#define compiled_rdb "../seq/identification/database/models/database.rdb"

// File paths
#define res_file    "../res/result_database_compiled_%03d.ppm"
...
Rox_Sint main(Rox_Sint argc, Rox_Char *argv[])
{
   ...
   Rox_Image model_image[nb_models] = {NULL, NULL};

   // Define the 2D model with texture and size in meters
   Rox_Model_Single_Plane model[nb_models] = {NULL, NULL};

   // Define odometry object
   Rox_Odometry_Single_Plane odometry[nb_models] = {NULL, NULL};
   Rox_Odometry_Single_Plane_Params params[nb_models] = {NULL, NULL};
   ...
   for (i = 0; i < nb_images; i++)
   {
      ...
      for(k = 0; k < nb_models; k++)
      {
         ...
         error = rox_ident_database_se3_getresult(&identified, pose, ident, k);
         if(error) goto function_terminate;
