...
int rox_neon_array2d_uchar_zncc_nomask (
   double * zncc,
...
)
{
   ...
   uint32x4_t neonmask = vdupq_n_u32(~0);
   ...
   for ( Rox_Sint i = 0; i < rows; i++)
   {
      ...
      for ( Rox_Sint j = 0; j < cols4; j++)
      {
         ...
         {
            neoncount = vaddq_u32(neoncount, neonone);
         }
...
)
{
   ...
   *zncc = nom / (sqrt(denom1)*sqrt(denom2));
#else
   error = rox_neon_array2d_uchar_zncc_nomask ( zncc, one_data, two_data, rows, cols );
   ROX_ERROR_CHECK_TERMINATE ( error );
#endif
