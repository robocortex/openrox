//==============================================================================
//
//    OPENROX   : File tracking_sl3.c
//
//    Contents  : Implementation of tracking_sl3 module
//
...
Rox_ErrorCode rox_tracking_sl3_new ( Rox_Tracking_SL3 * tracking, Rox_Tracking_Params params, Rox_Image model )
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   Rox_Tracking_SL3 ret = NULL;
   ...
   *tracking = NULL;
   ...
   ret = (Rox_Tracking_SL3) rox_memory_allocate(sizeof(*ret), 1);
   if (!ret)
   { error = ROX_ERROR_NULL_POINTER; ROX_ERROR_CHECK_TERMINATE ( error ); }
   ...
   error = rox_tracking_patch_sl3_new ( &ret->tracker ); 
   ROX_ERROR_CHECK_TERMINATE ( error );
   ...
   if (ret->parent.init_pyr < 0) ret->parent.init_pyr = 0;// (Rox_Sint) (ret->pyramid->count - 1);
   ...
   // Create a new plane search for prediction 
   ...
   ret->parent._fptr_make = (Rox_ErrorCode (*)(Rox_Tracking, Rox_Image)) rox_tracking_sl3_make;
   ret->parent._fptr_del = (Rox_ErrorCode (*)(Rox_Tracking *)) rox_tracking_sl3_del;
   ret->parent._fptr_set_mask = (Rox_ErrorCode (*) (Rox_Tracking, Rox_Imask)) rox_tracking_sl3_set_mask;

   *tracking = ret;

function_terminate:
   if (error) rox_tracking_sl3_del(&ret);

   return error;
}

Rox_ErrorCode rox_tracking_sl3_del(Rox_Tracking_SL3 *tracking)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   Rox_Tracking_SL3 todel = NULL;
   ...
   *tracking = 0;
   ...
   rox_tracking_patch_sl3_del(&todel->tracker);
...
Rox_ErrorCode rox_tracking_sl3_make ( Rox_Tracking_SL3 tracking, const Rox_Image image)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   ...
   for (Rox_Sint level = tracking->parent.init_pyr; level >= tracking->parent.stop_pyr; level--)
   {
      ...
      error = rox_tracking_patch_sl3_set_homography ( tracking->tracker, tracking->parent.zoom_homography ); 
      ROX_ERROR_CHECK_TERMINATE ( error );
      ...
      // Prediction at the init level of the pyramid
      ...
      // Compute ZNCC score 
      error = rox_patchplane_prepare_sl3 ( patch, tracking->tracker->homography, tracking->parent.normalized_cur ); 
      ROX_ERROR_CHECK_TERMINATE ( error );
      ...
      // Execute tracking
      error = rox_tracking_patch_sl3_make ( tracking->tracker, patch, tracking->parent.normalized_cur, tracking->parent.miter );  
      ROX_ERROR_CHECK_TERMINATE ( error );
      ...
      {
         error = rox_tracking_patch_sl3_get_homography ( tracking->parent.zoom_homography, tracking->tracker ); 
         ROX_ERROR_CHECK_TERMINATE ( error );
      }
      else
      {
...
Rox_ErrorCode rox_tracking_sl3_set_mask ( Rox_Tracking_SL3 tracking, const Rox_Imask mask )
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   Rox_PatchPlane lastlevel = 0;
