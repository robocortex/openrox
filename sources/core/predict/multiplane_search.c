...
#include <baseproc/image/remap/remap_bilinear_omo_float_to_float.h>
...
Rox_ErrorCode rox_multi_plane_search_new (
...
)
{
   ...
   *plane_search = NULL;
   ...
   error = rox_array2d_point2d_float_new(&ret->map, search_rows, search_cols);
   ROX_ERROR_CHECK_TERMINATE ( error );
...
function_terminate:
   if (error) rox_multi_plane_search_del(&ret);
   return error;
}

Rox_ErrorCode rox_multi_plane_search_del(Rox_Plane_Search * plane_search)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
...
Rox_ErrorCode rox_multi_plane_search_set_model (
   Rox_Plane_Search plane_search, 
...
)
{
   ...
   //error = rox_array2d_float_save("test_model.txt", plane_search->model);
   //ROX_ERROR_CHECK_TERMINATE ( error );
...
Rox_ErrorCode rox_multi_plane_search_set_model_warp (
   Rox_Plane_Search plane_search, 
...
)
{
   ...
   // error = rox_array2d_float_save("test_model_warp.txt", plane_search->model);
   // ROX_ERROR_CHECK_TERMINATE ( error );
...
Rox_ErrorCode rox_multi_plane_search_make (
   Rox_Plane_Search plane_search, 
...
)
{
   ...
   // TODOD : replace the two function calls with a single call. The map is allocated and deleted inside the function.
   if(1)
   {
      // Warp current image onto the search template
      error = rox_warp_grid_sl3_float(plane_search->map, plane_search->c_G_s);
      ROX_ERROR_CHECK_TERMINATE ( error );

      error = rox_remap_bilinear_omo_float_to_float(plane_search->search, plane_search->search_mask, current_image, plane_search->map);
      ROX_ERROR_CHECK_TERMINATE ( error );
   }
   else
   {
...
Rox_ErrorCode rox_multi_plane_search_get_results (
   Rox_Double * score, 
   Rox_Double * shift_u, 
   Rox_Double * shift_v, 
...
)
{
   ...
   error = rox_multi_plane_search_get_shift(shift_u, shift_v, plane_search);
   ROX_ERROR_CHECK_TERMINATE ( error );

   error = rox_multi_plane_search_get_score(score, plane_search);
   ROX_ERROR_CHECK_TERMINATE ( error );
...
Rox_ErrorCode rox_multi_plane_search_get_shift (
   Rox_Double * shift_u, 
   Rox_Double * shift_v, 
   const Rox_Plane_Search plane_search
)
...
Rox_ErrorCode rox_multi_plane_search_get_score (
   Rox_Double * score, 
   const Rox_Plane_Search plane_search
)
{
...
Rox_ErrorCode rox_multi_plane_search_update_pose_translation (
   Rox_MatSE3 c_T_r, 
...
Rox_ErrorCode rox_multi_plane_search_update_pose_rotation (
   Rox_MatSE3 c_R_r, 
   const Rox_MatUT3 K, 
   const Rox_Plane_Search plane_search
)
...
//    error = rox_dynvec_point3d_double_print(vref);
//    ROX_ERROR_CHECK_TERMINATE ( error );
//
//    error = rox_dynvec_point3d_double_print(vcur);
//    ROX_ERROR_CHECK_TERMINATE ( error );
...
function_terminate:
   ...
   rox_array2d_double_del(&r_H_t);
   rox_array2d_double_del(&c_H_t);

   return error;
}

Rox_ErrorCode rox_multi_plane_search_update_homography(Rox_MatSL3 c_G_r, const Rox_Plane_Search plane_search)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
