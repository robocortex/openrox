//==============================================================================
//
//    OPENROX   : File tracking_point.c
//
//    Contents  : Implementation of tracking_point module
//
...
//! To be commented
struct Rox_Tracking_Point_Struct
{
   //! To be commented
   Rox_Double pos_x;
   //! To be commented
...
Rox_ErrorCode rox_tracking_point_new(Rox_Tracking_Point * obj, Rox_Uint size)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   Rox_Tracking_Point ret = NULL;
   Rox_Uint bigsize;
   ...
   *obj = NULL;
   ...
   ret = (Rox_Tracking_Point) rox_memory_allocate(sizeof(struct Rox_Tracking_Point_Struct), 1);
...
function_terminate:
   if (error) rox_tracking_point_del(&ret);
   return error;
}

Rox_ErrorCode rox_tracking_point_del(Rox_Tracking_Point * obj)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   Rox_Tracking_Point todel;
...
Rox_ErrorCode rox_tracking_point_set_reference (
   Rox_Tracking_Point obj, Rox_Image source, Rox_Double pos_x, Rox_Double pos_y)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   ...
   if (pos_x < 1) {error = ROX_ERROR_BAD_SIZE; ROX_ERROR_CHECK_TERMINATE ( error );}
   if (pos_y < 1) {error = ROX_ERROR_BAD_SIZE; ROX_ERROR_CHECK_TERMINATE ( error );}

   Rox_Sint rows = 0, cols = 0;
   error = rox_array2d_uchar_get_rows(&rows, source); ROX_ERROR_CHECK_TERMINATE ( error );
   error = rox_array2d_uchar_get_cols(&cols, source); ROX_ERROR_CHECK_TERMINATE ( error );

   if (pos_y + bigsize - 1 > rows) {error = ROX_ERROR_BAD_SIZE; ROX_ERROR_CHECK_TERMINATE ( error );}
   if (pos_x + bigsize - 1 > cols) {error = ROX_ERROR_BAD_SIZE; ROX_ERROR_CHECK_TERMINATE ( error );}
...
Rox_ErrorCode rox_tracking_point_set_warped_reference (
   Rox_Tracking_Point obj,
   Rox_Image source,
   Rox_Double pos_u,
   Rox_Double pos_v,
   Rox_Double Z,
...
Rox_ErrorCode rox_tracking_point_set_referencepatch(Rox_Tracking_Point obj, Rox_Array2D_Float source)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
...
Rox_ErrorCode rox_tracking_point_set_predicted_position(Rox_Tracking_Point obj, Rox_Double pos_x, Rox_Double pos_y)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
...
Rox_ErrorCode rox_tracking_point_track(Rox_Tracking_Point obj, Rox_Image current, Rox_Double minscore)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
   Rox_Double zncc;

   if (!obj || !current) {error = ROX_ERROR_NULL_POINTER; ROX_ERROR_CHECK_TERMINATE ( error );}

   Rox_Double ** dsol = NULL;
   rox_array2d_double_get_data_pointer_to_pointer( &dsol, obj->solsys);

   for (Rox_Sint iter = 0; iter < 8; iter++)
   {
      error = rox_remap_bilinear_trans_uchar_to_float(obj->current, obj->curmask, current, (Rox_Float) (obj->current_pos_x - 1), (Rox_Float) (obj->current_pos_y - 1));
      ROX_ERROR_CHECK_TERMINATE ( error );

      error = rox_array2d_float_scaleshift(obj->currentlum, obj->current, 1.0, obj->luminosity_beta);
      ROX_ERROR_CHECK_TERMINATE ( error );

      rox_array2d_float_substract(obj->diff, obj->reference, obj->currentlum);
      rox_array2d_float_add(obj->meanlum, obj->currentlum, obj->reference);
      rox_array2d_float_scale(obj->meanlum, obj->meanlum, 0.5);

      error = rox_array2d_float_basegradient(obj->gradientx, obj->gradienty, obj->gradientmask, obj->meanlum, obj->curmask);
      ROX_ERROR_CHECK_TERMINATE ( error );

      error = linsys_texture_rxry_light_affine(obj->JtJ, obj->Jtf, obj->diff, obj->gradientx, obj->gradienty, obj->gradientmask);
      ROX_ERROR_CHECK_TERMINATE ( error );
   ...
   error = rox_array2d_float_zncc_nomask(&zncc, obj->current, obj->reference);
   ROX_ERROR_CHECK_TERMINATE ( error );

   if (zncc < minscore) {error = ROX_ERROR_NUMERICAL_ALGORITHM_FAILURE; ROX_ERROR_CHECK_TERMINATE(error)}

function_terminate:
   return error;
}

Rox_ErrorCode rox_tracking_point_get_current (
   Rox_Double * current_x, 
   Rox_Double * current_y, 
   Rox_Tracking_Point obj
)
{
   Rox_ErrorCode error = ROX_ERROR_NONE;

   if ( !current_x || !current_y || !obj ) 
   { error = ROX_ERROR_NULL_POINTER; ROX_ERROR_CHECK_TERMINATE(error); }
...
Rox_ErrorCode rox_tracking_point_get_reference_patch ( Rox_Image ref, Rox_Tracking_Point obj )
{
   Rox_ErrorCode error = ROX_ERROR_NONE;
